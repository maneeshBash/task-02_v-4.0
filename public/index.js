function promptUserForDetails() {
    const fields = ['title', 'firstName', 'lastName', 'email', 'streetNumber', 'streetName', 'city', 'state', 'country', 'postcode', 'phone', 'cell', 'dob', 'nat', 'picture'];
    const changes = {};

    fields.forEach(field => {
        if (confirm(`Do you want to update the ${field.replace(/([A-Z])/g, ' $1').toLowerCase()}?`)) {
            changes[field] = prompt(`Enter ${field.replace(/([A-Z])/g, ' $1')}:`);
        }
    });
    ...
    return {
        ...
        phone: changes.phone,
        cell: changes.cell,
        dob: { date: changes.dob },
        nat: changes.nat,
        picture: { thumbnail: changes.picture }
    };
}
...
async function fetchData(page = 1, limit = 10) {
    ...
    picture.src = user.picture?.thumbnail || '/images/default-thumbnail.png'; // Use default image if thumbnail is missing
    ...
    email.textContent = user.email || 'N/A'; // Default to 'N/A' if email is missing
